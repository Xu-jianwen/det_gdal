...
def bbox_iou(box1, box2, x1y1x2y2=True):
    ...
    # iou = inter_area /min(b1_area,b2_area)
...
class Dataset:
    ...
    def get_data_and_resolution(self, band):
#band 读取第几个通道的数据
        dataset = gdal.Open(self.in_file)
        ulx, pielwidthxx, xskew, uly, yskew, pixelheighty = dataset.GetGeoTransform()
        band = dataset.GetRasterBand(band)
        data = band.ReadAsArray()
        return data, pielwidthxx, pixelheighty
...
#获取经纬度信息
        ...
        map_x = gtf[0] + x * gtf[1] + y * gtf[2]
        map_y = gtf[3] + x * gtf[4] + y * gtf[5]
        return map_x, map_y
#-----------------------end----------------------#
def geo2lonlat(dataset):
    '''
